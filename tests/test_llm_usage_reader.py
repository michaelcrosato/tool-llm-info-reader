...
class LlmUsageReaderTests(unittest.TestCase):
    ...
    def test_record_rejects_cached_input_tokens_above_input_tokens(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp)
            code = self.run_cli(
                data_dir,
    ...
    def test_read_ledger_rejects_hash_valid_offset_started_at(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp)
            code = self.run_cli(
                data_dir,
            ...
            ledger = tool.ledger_path(data_dir)
            record = json.loads(ledger.read_text(encoding="utf-8"))
            record["started_at"] = "2026-06-18T20:00:00+00:00"
            tool.refresh_record_hash(record)
            ledger.write_text(json.dumps(record) + "\n", encoding="utf-8")

            with self.assertRaisesRegex(tool.CliError, "started_at.*canonical UTC"):
                tool.read_ledger(data_dir)

    def test_read_ledger_rejects_hash_valid_relative_created_at(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp)
            code = self.run_cli(
                data_dir,
            ...
            with self.assertRaisesRegex(tool.CliError, "created_at.*canonical UTC"):
                tool.read_ledger(data_dir)
    ...
    def test_read_ledger_rejects_hash_valid_unavailable_billing_with_provider_quantity(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp)
            code = self.run_cli(
                data_dir,
    ...
    def test_read_ledger_rejects_hash_valid_manual_billing_with_provider_line_item(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp)
            code = self.run_cli(
                data_dir,
            ...
            ledger = tool.ledger_path(data_dir)
            record = json.loads(ledger.read_text(encoding="utf-8"))
            record["billing"]["line_item"] = "Completions"
            tool.refresh_record_hash(record)
            ledger.write_text(json.dumps(record) + "\n", encoding="utf-8")

            with self.assertRaisesRegex(tool.CliError, "billing.line_item"):
                tool.read_ledger(data_dir)
    ...
    def test_read_ledger_rejects_manual_attestation_without_usage_or_reason(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp)
            code = self.run_cli(
                data_dir,
    ...
    def test_openai_usage_import_rejects_cached_tokens_above_input_tokens(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp) / "data"
            sample = Path(tmp) / "usage.json"
            sample.write_text(
    ...
    def test_finish_rejects_final_run_state_record_id_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp) / "data"
            run_id = "run_mismatched_record"
            self.assertEqual(
                self.run_cli(
                    data_dir,
                    "start",
            ...
            corrupt_state["record_id"] = "rec_0000000000000000"
    ...
    def test_finish_rejects_relative_run_state_started_at(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp) / "data"
            runs_dir = data_dir / "runs"
            runs_dir.mkdir(parents=True)
    ...
    def test_finish_rejects_offset_run_state_started_at(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp) / "data"
            runs_dir = data_dir / "runs"
            runs_dir.mkdir(parents=True)
    ...
    def test_finish_rejects_running_run_state_with_finished_at(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp) / "data"
            runs_dir = data_dir / "runs"
            runs_dir.mkdir(parents=True)
            (runs_dir / "runningdone.json").write_text(
                json.dumps(
                    {
                        "schema_version": tool.SCHEMA_VERSION,
                        "run_id": "runningdone",
    ...
    def test_finish_rejects_running_run_state_with_record_id(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp) / "data"
            runs_dir = data_dir / "runs"
            runs_dir.mkdir(parents=True)
            (runs_dir / "runningrecord.json").write_text(
                json.dumps(
                    {
                        "schema_version": tool.SCHEMA_VERSION,
                        "run_id": "runningrecord",
                        "provider": "openai",
                        "model": "gpt-5.4",
                        "started_at": "2026-06-18T20:00:00Z",
                        "status": "running",
    ...
    def test_finish_rejects_final_run_state_without_finished_at(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp) / "data"
            runs_dir = data_dir / "runs"
            runs_dir.mkdir(parents=True)
            (runs_dir / "finalmissingtime.json").write_text(
                json.dumps(
                    {
                        "schema_version": tool.SCHEMA_VERSION,
                        "run_id": "finalmissingtime",
                        "provider": "openai",
                        "model": "gpt-5.4",
                        "started_at": "2026-06-18T20:00:00Z",
                        "status": "completed",
    ...
    def test_finish_rejects_final_run_state_with_malformed_record_id(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp) / "data"
            runs_dir = data_dir / "runs"
            runs_dir.mkdir(parents=True)
            (runs_dir / "finalbadrecord.json").write_text(
                json.dumps(
                    {
                        "schema_version": tool.SCHEMA_VERSION,
                        "run_id": "finalbadrecord",
                        "provider": "openai",
                        "model": "gpt-5.4",
                        "started_at": "2026-06-18T20:00:00Z",
                        "finished_at": "2026-06-18T20:01:00Z",
    ...
    def test_finish_rejects_run_state_finished_before_started(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            runs_dir = data_dir / "runs"
            runs_dir.mkdir(parents=True)
            (runs_dir / "badorder.json").write_text(
                json.dumps(
                    {
                        "schema_version": tool.SCHEMA_VERSION,
                        "run_id": "badorder",
                        ...
                        "model": "gpt-5.4",
                        "started_at": "2026-06-18T20:00:00Z",
                        "finished_at": "2026-06-18T19:59:59Z",
                        "status": "completed",
                        "record_id": "rec_0000000000000000",
                    }
                ),
    ...
    def test_fetch_openai_validates_payloads_before_export_writes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp) / "data"
            calls = []
